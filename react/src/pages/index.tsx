...
const Nirdharanam = dynamic(() => import('components/Nirdharanam'), {
  ssr: false
});
const TimeControl = dynamic(() => import('components/TimeControl'), {
  ssr: false
});
...
const Home: NextPage = () => {
  ...
  const nirdharanam = useAtomValue(nirdhAraNState);
  // end states
  let collect: number[] = [];
  ...
  const todani = (n: number) => {
    ...
    if (chalan) {
      ...
      } else if (jn == '22212') set_nirdharan(true);
      if (collect.length === 5) {
        collect = [];
        blink();
      }
  ...
  const check = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (prachalan) {
      key_num.current++;
      setTimeout(() => {
        if (key_num.current == 0) return;
        else if (key_num.current == 1) setValue('');
        key_num.current--;
      }, 3000);
    } else setValue('');
  };
  ...
  return (
    <div
      ref={body}
      className={`body w-screen h-screen ${
        JSON.stringify(data) !== '{}' ? 'bg-black' : 'bg-stone-900'
      } select-none p-2 fixed top-0`}
    >
      <Head>
      ...
      <>
        <div className={prachalan ? '' : 'hidden'}>
          ...
          >
            <input
              ...
              value={value}
              type="text"
              className="w-52 ml-0 text-teal-500 bg-black block font-semibold m-1 text-xl border-2 border-gray-900 rounded-md"
            />
          </form>
          <Select
            option={selectValue}
            onChange={(e: ChangeEvent<HTMLSelectElement>) => {
              set_media_file(e.target.value);
            }}
          />
        </div>
        <div className="fixed z-50 top-2.5 left-[225px]">
          <button
            onClick={() => todani(1)}
            onDoubleClick={() => lasan && set_media_show(-1)}
            className="sam bg-[#ff0] border-transparent border-2 font-bold py-1.5 px-2 rounded-md focus:border-red-500 focus:outline-none active:border-blue-600 active:bg-amber-200"
          >
            ௰
          </button>
          <button
            onClick={() => todani(2)}
            onDoubleClick={() => chalan && lasitam(false)}
            className="sam bg-[greenyellow] ml-4 border-transparent border-2 font-bold py-1.5 px-2 rounded-md focus:border-red-500 focus:outline-none active:border-blue-600 active:bg-lime-300"
          >
            ௲
          </button>
        </div>
      ...
      <>
        {lasan && (
          <div className={media_show ? '' : 'hidden'}>
            <TimeControl elm={media_ref} />
            {(() => {
              let dt = file[1];
              if (!lasan || dt[0] === '') return <></>;
...
const Select: NextPage<{
  ...
  return (
    <select
      value={option}
      onChange={onChange}
      className="w-5 h-4 mt-2 text-white bg-black outline-none border-2 rounded-sm border-gray-900"
    >
      {' '}
      {itemElm}
    </select>
